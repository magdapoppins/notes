...
const CLI = () => {
  ...
  const [log, setLog] = React.useState([
    bakeToLogMessage("Type 'help' to get started.", "info"),
  ]);
  ...
  const getResponse = (e) => {
    ...
    if (input === "help") {
      ...
      newLog.push(
        bakeToLogMessage(
          "Try listing contents (ls), moving into folders (cd directoryname) and copying texts (cat filename).",
          "info"
        )
      );
      setLog(newLog);
    ...
    if (input.startsWith("cat ")) {
      ...
      if (dir.files[filename]) {
        setLog([
          ...log,
          bakeToLogMessage(
            `Users/magda/home/guest/${location} 🛼 cat ${filename}`
          ),
